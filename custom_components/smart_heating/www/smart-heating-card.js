...
class SmartHeatingCard extends HTMLElement {
  ...
  render(){
    ...
    const s1EntityId=a.switch_1||this.config?.switch_1;const s1Entity=s1EntityId?this.state(s1EntityId):null;let contact1Closed=false;if(s1Entity&&s1Entity.state!=='unavailable'&&s1Entity.state!=='unknown'){contact1Closed=(s1Entity.state==='on');}else if(a.contact_1_state!==undefined){contact1Closed=Boolean(a.contact_1_state);}else if(a.heating!==undefined){contact1Closed=Boolean(a.heating);}else{contact1Closed=Boolean(a.hvac_action==='heating'||c?.state==='heat');}const showFlame=Boolean(contact1On&&contact1Closed&&effectEnabled);const flameDim=showFlame&&!enabled;const heating=enabled&&contact1Closed;
