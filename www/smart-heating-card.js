...
const SH_DEFAULTS = {
  ...
  effect_enabled: true, effect_x: 0, effect_y: 0, effect_s: 1, effect_opacity: 0.85,
};
...
<button class="sh-reset-btn" data-reset-labels type="button">${tr('reset_labels')}</button>
    ...
    root.querySelectorAll('[data-label-toggle]').forEach(btn=>btn.addEventListener('click',()=>{
      const key=btn.dataset.labelToggle;
      const cur=SH_GET_STORE_JSON('smart_heating_labels_visible');
      cur[key]=!(cur[key]!==false);
