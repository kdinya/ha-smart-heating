...
const SH_DEFAULTS = {
  screen_aspect_ratio: 1.5, card_radius: 27, row_offset_y: 0, frame_width: 2.4,
  header_inner_y: 0, line_y: 0,
  brand_group_x: 0, brand_group_y: 0, date_group_x: 0, date_group_y: 0,
  dial_y: -6,
  room_letter_spacing: -5, room_decimal_size: 1, room_unit_size: 1,
  target_letter_spacing: 0, target_decimal_size: 1, target_unit_size: 1, target_unit_gap: 4,
  adjust_gap: 42, adjust_icon_size: 32, adjust_button_size: 66,
  panel_s: 0.89, panel_h: 0, panel_gap: 5.5, panel_y: -3, panel_button_size: 1,
  effect_enabled: true, effect_x: 0, effect_y: -2, effect_s: 1.5, effect_opacity: 0.85,
};
...
class SmartHeatingCard extends HTMLElement {
  ...
  setConfig(config){if(!config||!config.entity)throw new Error('Smart Heating: вкажіть climate-ентіті / define a climate entity');this.config={...config};this._menuOpen=Boolean(this._menuOpen);this._programmerOn=this._programmerOn??Boolean(SH_READ_STORE('programmer_on',false));this._menuTab=this._menuTab||'overview';if(!this.shadowRoot)this.attachShadow({mode:'open'});this.render();}
  ...
  render(){
    ...
    const n=(key,fallback=0)=>Number.isFinite(Number(this.config[key]))?Number(this.config[key]):(Object.prototype.hasOwnProperty.call(SH_DEFAULTS,key)?SH_DEFAULTS[key]:fallback);const g=(id)=>{const key=id.replaceAll('-','_');return `--${id}-x:${n(key+'_x')};--${id}-y:${n(key+'_y')};--${id}-s:${SH_CLAMP(n(key+'_s',1),.25,4,1)};`};const groupVars=SH_GROUPS.map(g).join('');const itemVars=SH_ITEMS.map(g).join('');
    ...
    const switch2Entity = this.config.switch_2 || c?.attributes?.switch_2;
    const contact1On = enabled;
    const contact2On = switch2Entity && this._hass?.states?.[switch2Entity] ? (this._hass.states[switch2Entity].state === 'on') : Boolean(this._programmerOn);
...
<style data-layout-override>ha-card{aspect-ratio:var(--card-ratio,1.5);border-radius:var(--card-radius,27px);overflow:hidden}.screen{flex:1 1 0;min-height:0;aspect-ratio:auto}.screen-grid{position:relative;width:100%;height:100%;grid-template-columns:30% 40% 30%;grid-template-rows:100%;gap:0;align-items:center}</style><ha-card class="${enabled?'is-enabled':'is-disabled'}" style="--card-ratio:${cardRatio};--screen-ratio:${screenRatio};${groupVars}${itemVars}--room-letter-spacing:${roomLetterSpacing}px;--room-decimal-size:${roomDecimalSize};--room-unit-size:${roomUnitSize};--target-letter-spacing:${targetLetterSpacing}px;--target-decimal-size:${targetDecimalSize};--target-unit-size:${targetUnitSize};--target-unit-gap:${targetUnitGap}px;--adjust-icon-size:${adjustIconSize}px;--row-offset:${rowOffset};--card-radius:${cardRadius}px;--frame-width:${frameWidth};--header-inner-y:${n('header_inner_y')};--brand-group-x:${n('brand_group_x')};--brand-group-y:${n('brand_group_y')};--date-group-x:${n('date_group_x')};--date-group-y:${n('date_group_y')};--line-y:${n('line_y')};--adjust-gap:${adjustGap}px;--adjust-button-size:${buttonSize}px;--panel-h:${SH_CLAMP(this.config.panel_h,-20,40,SH_DEFAULTS.panel_h)};--effect-x:${n('effect_x')};--effect-y:${n('effect_y',-2)};--effect-s:${SH_CLAMP(n('effect_s'),.25,2.5,SH_DEFAULTS.effect_s)};--effect-opacity:${SH_CLAMP(n('effect_opacity'),.1,1,SH_DEFAULTS.effect_opacity)};--panel-gap:${SH_CLAMP(n('panel_gap'),0,20,SH_DEFAULTS.panel_gap)};--panel-button-size:${SH_CLAMP(n('panel_button_size'),.5,2,SH_DEFAULTS.panel_button_size)}"><div class="device"><section class="screen"><div class="header-shell"><div class="group-header"><div class="brand"><div class="flame brand-icon">♨</div><div class="brand-name"><b>${this.safe(this.config.title||'HEAT')}</b><small>GAS BOILER</small></div></div><div class="clock"><span class="date">${date}</span><strong class="clock-time">${time}</strong></div><span class="signal"><i></i><i></i><i></i><i></i></span></div><div class="line"></div></div><div class="screen-grid"><div class="side group-weather"><div class="metric element-outdoor" style="display:${outdoorVisible?'':'none'}"><div class="metric-icon weather-icon"><svg viewBox="0 0 32 32"><path d="M14 4a3 3 0 0 1 6 0v14.6a6 6 0 1 1-6 0Z"/><path d="M17 11v9.6"/><path d="M11 9h1.5M11 13h1.5M11 17h1.5"/></svg></div><div><label>${tr('outdoor')}</label><strong>${this.safe(outdoor)} <em>°C</em></strong></div></div><div class="metric element-wind" style="display:${windVisible?'':'none'}"><div class="metric-icon weather-icon"><svg viewBox="0 0 32 32"><path d="M3 11h16c3 0 3-5 0-5-1.4 0-2.4.7-3 1.8M3 16h22c4 0 4 6 0 6-1.7 0-2.8-.8-3.5-2M3 21h10"/></svg></div><div><label>${tr('wind')}</label><strong>${this.safe(wind)}</strong></div></div><div class="metric element-rain" style="display:${rainVisible?'':'none'}"><div class="metric-icon weather-icon"><svg viewBox="0 0 32 32">${rainIcon}</svg></div><div><label>${tr('rain')}</label><strong>${this.safe(rain)}</strong></div></div></div><div class="center group-climate"><div class="dial ${heating&&effectEnabled?'is-heating':''}"><div class="flame-effect ${heating&&effectEnabled?'is-active':''}"><svg viewBox="0 0 100 150" aria-hidden="true"><path class="flame-outer" d="M50 150C18 150 4 126 13 99c5-15 19-25 30-41 8-12 9-27 7-45 28 20 43 43 40 70-1 9-4 17-9 24 0-20-8-34-22-45 4 24-14 34-8 52 5 16 19 25 39 29-9 5-19 7-40 7Z"/><path class="flame-mid" d="M50 137c-19 0-27-14-22-29 3-9 11-15 17-24 5-7 6-15 5-25 16 13 25 27 23 43-1 8-4 14-9 19 0-10-4-17-11-23 2 13-7 20-3 28 3 6 8 10 16 12-4 0-9-1-16-1Z"/><path class="flame-inner" d="M50 127c-9 0-14-7-11-15 2-5 6-9 9-14 3-4 3-8 3-13 8 7 12 15 11 24 0 5-2 8-5 11 0-5-2-9-5-12 0 7-4 12-2 15 2 3 5 4 9 5-3 0-6-1-9-1Z"/></svg></div><div class="dial-content"><div class="temp"><span class="temp-int" style="display:${this.config.room_int_visible!==false?'':'none'}">${this.safe(roomInt)}</span><span class="temp-dec" style="display:${this.config.room_dec_visible!==false?'':'none'}">${roomDec?'.'+this.safe(roomDec):''}</span><sup class="temp-unit" style="display:${this.config.room_unit_visible!==false?'':'none'}">°C</sup></div><div class="target"><strong><span class="target-int">${this.safe(targetInt)}</span><span class="target-dec" style="display:${this.config.target_dec_visible!==false?'':'none'}">${targetDec?'.'+this.safe(targetDec):''}</span><span class="target-unit" style="display:${this.config.target_unit_visible!==false?'':'none'}">°C</span></strong></div></div></div><div class="adjust"><button data-delta="-0.5">−</button><button data-delta="0.5">+</button></div></div><div class="side"><div class="metric orange element-humidity group-humidity" style="display:${humidityVisible?'':'none'}"><div class="metric-icon">♢</div><div><label>${tr('humidity')}</label><strong class="humidity-value"><span class="humidity-int">${this.safe(String(humidity).split('.')[0])}</span><span class="humidity-dec">${String(humidity).includes('.')?'.'+String(humidity).split('.')[1]:''}</span><em class="humidity-unit">%</em></strong></div></div><div class="scheme-title element-scheme">${tr('scheme')}</div><div class="scheme"><div class="scheme-card ${contact1On?'active':''}" data-scheme-toggle="1" role="button" tabindex="0"><div class="scheme-icon"><svg viewBox="0 0 24 24"><path d="M12 3c-1.6 4.5-5.3 5.5-5.3 9.2A5.3 5.3 0 0 0 12 17.5a5.3 5.3 0 0 0 5.3-5.3C17.3 8.5 13.6 7.5 12 3Z"/></svg></div><div class="scheme-number">1</div><span>${tr('contact1')}</span></div><div class="scheme-card ${contact2On?'active':''}" data-scheme-toggle="2" role="button" tabindex="0"><div class="scheme-icon"><svg viewBox="0 0 24 24"><rect x="5" y="4" width="14" height="16" rx="2"/><rect x="9" y="8" width="6" height="5"/><path d="M8 17h8"/></svg></div><div class="scheme-number">2</div><span>${tr('contact2')}</span></div></div></div></div></section><nav class="control-panel group-panel"><div class="control power ${enabled?'active':''}" data-power><span class="button"><svg viewBox="0 0 24 24"><path d="M12 3v8m-5.7-5A9 9 0 1 0 17.7 6"/></svg></span>${tr('power')}</div><div class="control"><span class="button"><svg viewBox="0 0 24 24"><path d="M12 3c-1.6 4.5-5.3 5.5-5.3 9.2A5.3 5.3 0 0 0 12 17.5a5.3 5.3 0 0 0 5.3-5.3C17.3 8.5 13.6 7.5 12 3Z"/></svg></span>${tr('program')}</div><div class="control fire"><span class="button"><svg viewBox="0 0 24 24"><path d="M12 21c4 0 6.2-2.7 6.2-6.2 0-3.8-2.9-5.2-4.2-8.8-1.7 1.5-2.2 3-2 4.7-2.5-1.3-3.1-3.2-3-5.7C6.7 7.8 5.8 10.1 5.8 13.5 5.8 18.1 8.4 21 12 21Z"/></svg></span><small>${heating?tr('boiler'):tr('waiting')}</small></div><div class="control"><span class="button"><svg viewBox="0 0 24 24"><path d="M4 19v-5m5 5V9m5 10V5m5 14v-8"/></svg></span>${tr('history')}</div><div class="control settings"><span class="button"><svg viewBox="0 0 24 24"><path d="M12 2.8l1 2.1c.6.1 1.2.4 1.8.7l2.1-1 1.5 1.5-1 2.1c.3.6.6 1.2.7 1.8l2.1 1v2l-2.1 1c-.1.6-.4 1.2-.7 1.8l1 2.1-1.5 1.5-2.1-1c-.6.3-1.2.6-1.8.7l-1 2.1h-2l-1-2.1c-.6-.1-1.2-.4-1.8-.7l-2.1 1-1.5-1.5 1-2.1c-.3-.6-.6-1.2-.7-1.8l-2.1-1v-2l2.1-1c.1-.6.4-1.2.7-1.8l-1-2.1 1.5-1.5 2.1 1c.6-.3 1.2-.6 1.8-.7l1-2.1h2Z"/><circle cx="12" cy="12" r="3.2"/></svg></span>${tr('settings')}</div></nav></div>${this._confirmDialog?`<div class="modal-backdrop confirm-backdrop" style="z-index:1100"><section class="modal" tabindex="-1" style="max-width:380px"><button class="modal-close confirm-cancel">×</button><div class="modal-title" style="color:var(--orange)">${this._confirmDialog.title}</div><div style="margin:14px 0 10px;font-size:clamp(12px,1.2cqw,16px);line-height:1.45;color:#d8e2ea">${this._confirmDialog.message}</div><div style="margin:10px 0 18px;padding:9px 12px;background:#ff8a0015;border-left:3px solid var(--orange);border-radius:6px;font-size:clamp(11px,1.05cqw,14px);color:#ffaa44;line-height:1.4">${this._confirmDialog.impact}</div><div style="display:flex;gap:10px;justify-content:flex-end;margin-top:14px"><button class="confirm-cancel" style="padding:8px 16px;border-radius:8px;border:1px solid #4f5d68;background:#182228;color:#c8d2dc;cursor:pointer;font-weight:600">Скасувати</button><button class="confirm-ok" style="padding:8px 18px;border-radius:8px;border:none;background:var(--orange);color:#0a1014;cursor:pointer;font-weight:700">Підтвердити</button></div></section></div>`:''}${this._menuOpen?`<div class="modal-backdrop"><section class="modal" tabindex="-1"><button class="modal-close">×</button><div class="modal-title">${tr('settings')}</div><div class="tabs"><div class="menu-section ${tab==='overview'?'open':''}"><button data-menu-section="overview" class="menu-head"><span>◉</span><b>${tr('overview')}</b><i>${tab==='overview'?'▲':'▼'}</i></button>${tab==='overview'?`<div class="menu-body"><b>Smart Heating</b><p>${tr('hint_overview')}</p></div>`:''}</div><div class="menu-section ${tab==='appearance'?'open':''}"><button data-menu-section="appearance" class="menu-head"><span>▣</span><b>${tr('appearance')}</b><i>${tab==='appearance'?'▲':'▼'}</i></button>${tab==='appearance'?`<div class="menu-body"><b>${tr('title_appearance')}</b><p>${tr('hint_appearance')}</p></div>`:''}</div><div class="menu-section ${tab==='entities'?'open':''}"><button data-menu-section="entities" class="menu-head"><span>⌘</span><b>${tr('entities')}</b><i>${tab==='entities'?'▲':'▼'}</i></button>${tab==='entities'?`<div class="menu-body"><b>${tr('title_entities')}</b><p>${tr('hint_entities')}</p></div>`:''}</div><div class="menu-section ${tab==='language'?'open':''}"><button data-menu-section="language" class="menu-head"><span>文</span><b>${tr('language')}</b><i>${tab==='language'?'▲':'▼'}</i></button>${tab==='language'?`<div class="menu-body"><div class="popup-choice"><button data-popup-lang="uk" class="${lang==='uk'?'active':''}">Українська</button><button data-popup-lang="en" class="${lang==='en'?'active':''}">English</button></div></div>`:''}</div></div></section></div>`:''}</ha-card>`;
    ...
    root.querySelector('.control.settings')?.addEventListener('click',()=>{this._menuOpen=true;this.render()});
    root.querySelector('[data-scheme-toggle="1"]')?.addEventListener('click', () => {
      const willBeOn = !contact1On;
      const impact = willBeOn 
        ? (contact2On ? 'Обидва контакти будуть увімкнені та працюватимуть разом (дублювання).' : 'Котел керуватиметься автоматикою картки за датчиком температури.')
        : (contact2On ? 'Автоматика картки вимкнеться. Котел працюватиме тільки від старого програматора.' : 'Обидва контакти вимкнені: картка не керуватиме котлом, а лише вимірюватиме температуру.');
      this._confirmDialog = {
        title: willBeOn ? 'Увімкнення Контакту 1' : 'Вимкнення Контакту 1',
        message: willBeOn 
          ? 'Увімкнути Контакт 1 (автоматичний термостат)? Котел буде автоматично вмикатися та вимикатися за датчиком температури та гістерезисом картки.'
          : 'Вимкнути Контакт 1 (автоматичний термостат)? Автоматичне керування котлом буде зупинено.',
        impact,
        action: () => {
          this._hass.callService('climate', 'set_hvac_mode', {
            entity_id: this.config.entity,
            hvac_mode: willBeOn ? 'heat' : 'off'
          });
        }
      };
      this.render();
    ...
    root.querySelector('[data-scheme-toggle="2"]')?.addEventListener('click', () => {
      ...
      const impact = willBeOn 
        ? (contact1On ? 'Обидва контакти будуть увімкнені та працюватимуть разом (дублювання).' : 'Котел керуватиметься виключно старим програматором. Автоматика картки не втручатиметься.')
        : (contact1On ? 'Старий програматор відключено. Котел керуватиметься автоматикою картки.' : 'Обидва контакти вимкнені: картка не керуватиме котлом, а лише вимірюватиме температуру.');
      this._confirmDialog = {
        title: willBeOn ? 'Увімкнення Контакту 2' : 'Вимкнення Контакту 2',
        message: willBeOn 
          ? 'Підключити Контакт 2 (старий програматор)? Лінію старого програматора буде задіяно вручну.'
          : 'Відключити Контакт 2 (старий програматор)? Лінію старого програматора буде вимкнено.',
        impact,
        action: () => {
          this._programmerOn = willBeOn;
          SH_WRITE_STORE('programmer_on', willBeOn);
          if (switch2Entity) {
            this._hass.callService('switch', willBeOn ? 'turn_on' : 'turn_off', { entity_id: switch2Entity });
          }
        }
      };
      this.render();
...
const SH_EDITOR_EN = {
  ...
  'Варіанти підключення': 'Connection modes',
...
class SmartHeatingCardEditor extends HTMLElement {
  ...
  _toggleSection(k){this._open[k]=!this._open[k];SH_WRITE_STORE(SH_OPEN_KEY,JSON.stringify(this._open));this.render()}
  ...
  _section(id,icon,title,body){return `<section class="accordion ${this._open[id]?'open':''}"><button class="section-head" data-section="${id}"><span class="section-icon">${icon}</span><span class="section-title">${this._ui(title)}</span><span class="section-arrow">${this._open[id]?'▲':'▼'}</span></button>${this._open[id]?`<div class="section-body">${body}</div>`:''}</section>`}
  render(){
    ...
    const general=`<div class="field"><label>${this._ui('Ентіті пристрою')}</label><ha-entity-picker id="entity"></ha-entity-picker></div>`
      +this._field('Назва пристрою','title',c.title,'HEAT')
      +`<div class="field"><label>${this._ui('Мова інтерфейсу')}</label><div class="choices"><button data-editor-lang="uk" class="${lang==='uk'?'active':''}">Українська</button><button data-editor-lang="en" class="${lang==='en'?'active':''}">English</button></div></div>`;
  ...
  _bind(){
    ...
    root.querySelectorAll('[data-editor-lang]').forEach(b=>b.onclick=()=>this._set('language',b.dataset.editorLang));
