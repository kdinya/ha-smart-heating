...
class SmartHeatingConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            entity_keys = (
                CONF_ROOM_TEMPERATURE, CONF_SWITCH_1, CONF_SWITCH_2,
                CONF_HUMIDITY, CONF_OUTDOOR_TEMPERATURE, CONF_WIND, CONF_PRECIPITATION,
            )
            chosen = [user_input[k] for k in entity_keys if user_input.get(k)]
