...
У HACS-репозиторії типу **Integration** адреса `/hacsfiles/ha-smart-heating/...` не є надійною: HACS не зобов'язаний публікувати папку `www` інтеграції як frontend-ресурс. Саме тому картка тепер використовує `/local`, який Home Assistant гарантовано обслуговує з `/config/www`.
