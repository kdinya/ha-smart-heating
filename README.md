...
3. використовує стандартну адресу `/hacsfiles/ha-smart-heating/smart-heating-card.js?v=1.0.2&build=reference-dashboard-v102-hacs2`.
...
```text
/hacsfiles/ha-smart-heating/smart-heating-card.js?v=1.0.2&build=reference-dashboard-v102-hacs2
```
...
After a HACS update, restart Home Assistant completely. The integration automatically finds old Smart Heating resources by filename, updates one to `/hacsfiles/ha-smart-heating/smart-heating-card.js?v=1.0.2&build=reference-dashboard-v102-hacs2`, and removes duplicates. If the old card is still displayed, refresh the browser with `Ctrl+F5` or clear the browser cache.
