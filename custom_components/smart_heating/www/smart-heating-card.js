...
<div class="sh-hyst-group">
  <div class="sh-hyst-label"><span>${tr('temp_step')}</span><span class="sh-hyst-val">${Number(currentTempStep).toFixed(1)} °C</span></div>
...
<button class="sh-reset-btn" data-reset-labels type="button">${tr('reset_labels')}</button>
    ...
    root.querySelectorAll('[data-temp-step-slider]').forEach(sl=>sl.addEventListener('input',e=>applyTempStep(e.target.value)));
